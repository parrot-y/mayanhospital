...
const carouselImages = [
    '/assets/images/pharmacy_counter.webp',
    '/assets/images/process_carousel_2.webp'
];

const ProcessImageCarousel = () => {
        ...
        <div className="relative w-full h-full">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                {carouselImages.map((_, i) => (
                    <div
                        key={i}
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${i === currentIndex ? 'bg-ovicare-primary w-6' : 'bg-white/30'}`}
                    />
                ))}
            </div>
        </div>
        ...
        <section ref={ref} className="bg-ovicare-dark text-white">
            <div className="container">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
                    ...
                    {/* Right: Sticky Image Carousel */}
                    <div className="lg:sticky lg:top-40">
                        <motion.div
                            initial={{ opacity: 0, scale: 0.95, filter: "blur(10px)" }}
                            animate={isVisible ? { opacity: 1, scale: 1, filter: "blur(0px)" } : {}}
                            transition={{ duration: 1.2, ease: [0.25, 0.8, 0.25, 1] }}
                            className="relative aspect-[4/3] md:aspect-square rounded-[40px] overflow-hidden group shadow-2xl bg-ovicare-dark/30"
                        >
                            <ProcessImageCarousel />
                            <div className="absolute bottom-8 right-8 w-24 h-24 bg-ovicare-primary rounded-3xl rotate-12 flex items-center justify-center z-20">
                                <img
